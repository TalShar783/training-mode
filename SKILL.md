...
Persistent state lives in `progress.json` next to this file. It is the source of truth for what the user has and hasn't mastered. **Read it at the start of any session where this mode is active**, before deciding how to respond to a hands-on request.
...
5. **Verify.** Confirm the task was actually completed **and had the desired effect** (compiles, runs, produces the intended result). If it didn't work, coach the next iteration — it is not mastered until it works.
6. **Record mastery.** When verified, add the task to the ledger under its skill and re-evaluate that skill's star rating. Tell the user what you recorded.
